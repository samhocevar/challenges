...
static uint32_t explore(uint32_t const *costs, uint32_t t, uint32_t stash = 0x0, uint32_t robots = ORE)
{
    ...
    {
        uint32_t dt = eta(costs[1], ore(stash), ore(robots));
        if (t > dt)
    ...
    {
        uint32_t dt = eta(costs[2], ore(stash), ore(robots));
        if (t > dt)
    ...
    {
        uint32_t dt = std::max(eta(costs[3], ore(stash), ore(robots)), eta(costs[4], cly(stash), cly(robots)));
        if (t > dt)
    ...
    if (obs(robots))
    {
        uint32_t dt = std::max(eta(costs[5], ore(stash), ore(robots)), eta(costs[6], obs(stash), obs(robots)));
        if (t > dt)

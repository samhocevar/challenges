...
# If we spend X milliseconds charging, we acquire speed X, and can then travel a
# distance of X × (time - X) millimeters.
# In order to beat the record, we therefore need to solve: X² - time × X + dist < 0.
# This function solves the quadratic equation for X the old way, using Δ = b²/4 - ac.
# The two resulting roots are: x1, x2 = -b/2 ± √Δ.
...
# Part 1: call compute on (t1, d1), (t2, d2), (t3, d3)… and multiply them all
